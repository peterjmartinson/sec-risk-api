...
class PeerDiscoveryService:
    ...
    def find_peers_for_ticker(self, ticker: str, top_n: int = 10) -> List[str]:
        ...
        target_subs = self.get_company_submissions(cik, write_cache=False)
        ...
        if not target_sic:
            return []
        # First query DB for existing peers with this SIC
        db_peers = get_peers_by_sic(self.db_path, target_sic, limit=top_n)
        peers: List[str] = [p["ticker"].upper() for p in db_peers]
        if len(peers) >= top_n:
            return peers[:top_n]

        # Otherwise, iterate over company_tickers and fetch missing SICs, upserting into DB
        entries = self.fetch_company_tickers()
        for e in entries:
            t = e.get("ticker")
            ...
            if t_up == ticker.upper():
                continue
            # If already in peers list from DB, skip
            if t_up in peers:
                continue
            # Try to fetch submissions but do NOT write per-company JSON files to disk
            try:
            ...
            # Upsert minimal info into DB and avoid saving JSON files
